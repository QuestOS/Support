	    ...
	    unsigned long load_flags)
{
  ...
  if (load_flags & KERNEL_SPI)
	...
	if (suggested_type == KERNEL_TYPE_NONE) {
		grub_printf ("loading big linux...\n");
		ret = grub_load_linux (kernel, arg);
	}
	else if (suggested_type == KERNEL_TYPE_MULTIBOOT) {
		grub_printf ("loading multiboot kernel...\n");
		ret = grub_load_multiboot (kernel, arg);
	}
...
load_module (char *module, char *arg)
{
  ...
  if (! len) {
    grub_printf ("Module %d read failed\n", mbi.mods_count);
    grub_close ();
    return 0;
  }

  grub_printf ("[Multiboot-module @ 0x%x, 0x%x bytes]\n", cur_addr, len);
