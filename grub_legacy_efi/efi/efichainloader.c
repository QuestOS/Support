...
grub_chainloader (char *filename)
{
  ...
  status = Call_Service_6 (b->load_image, 1, grub_efi_image_handle, file_path,
			   NULL, 0, &image_handle);
  if (status != GRUB_EFI_SUCCESS)
    {
      errnum = ERR_UNRECOGNIZED;
      ...
      if (status == GRUB_EFI_ACCESS_DENIED)
				grub_printf ("access denied\n");
			else if (status == GRUB_EFI_NO_RESPONSE)
				grub_printf ("no response\n");
